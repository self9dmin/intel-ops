...
export const MISSIONS: Mission[] = [
  ...
  {
    id: "mission-ghost-in-the-trace",
    title: "Ghost in the Trace",
    codename: "TRACE HUNT",
    role: "SRE",
    difficulty: "elite",
    description: "A checkout flow is degrading. No alert fired. No obvious error. The problem is hiding in the span waterfall. Find it.",
    briefing: "You got a Slack message: astroshop-checkout is slow. P99 latency spiked. No Davis problem opened. No errors in logs. The ghost is somewhere in the distributed trace. Your job: open the Distributed Tracing app, find the slowest trace, read the waterfall, and identify where the latency is being introduced. No Assist. No hints. Move. Use the Dynatrace Playground at https://playground.apps.dynatrace.com",
    timerSeconds: 480,
    status: "available",
    prerequisites: [],
    disciplines: [
    ...
    topics: ["traces", "services", "dt-intelligence"],
    category: "root-cause-analysis",
    apps: ["Distributed Tracing"],
    checkpoints: [
      {
        id: "cp1",
        title: "Open Distributed Tracing",
        instruction: "Open the Distributed Tracing app (Apps → Application Observability → Distributed Tracing). Filter by service 'astroshop-checkout'. Which filter allows you to narrow traces by whether the request succeeded or failed?",
        hint: "Look at the filter options at the top of the Distributed Tracing app. One filter specifically targets the outcome of the request.",
        type: "multiple-choice",
        choices: [
          "Request status",
          "Span kind",
        ...
        correctChoice: "Request status",
        points: 150,
      },
      {
        id: "cp2",
        title: "Find the Slowest Trace",
        instruction: "Sort traces by duration descending. Open the slowest trace for astroshop-checkout. What is the endpoint name of the root span?",
        hint: "The root span is the top-level span in the waterfall — it represents the entry point of the entire request. The endpoint name is shown in the span details panel.",
        type: "multiple-choice",
        choices: [
          "PlaceOrder",
          "GetCart",
        ...
        correctChoice: "PlaceOrder",
        points: 200,
      },
      {
        id: "cp3",
        title: "Read the Waterfall",
        instruction: "In the trace waterfall view, how many total spans are visible in the slowest astroshop-checkout trace?",
        hint: "The waterfall shows every span in the trace as a horizontal bar. Count all spans shown — the number is displayed in the trace detail header.",
        type: "multiple-choice",
        choices: [
          "Less than 20",
          "20 to 40",
        ...
        correctChoice: "41 to 80",
        points: 200,
      },
      {
        id: "cp4",
        title: "Identify the Protocol",
        instruction: "Click the root span of the slowest astroshop-checkout trace. What RPC framework is shown in the span attributes?",
        hint: "The span attributes panel shows technical details about how this service communicates. Look for the RPC framework field.",
        type: "multiple-choice",
        choices: [
          "REST",
          "grpc",
  ...
  {
    id: "mission-topology-under-fire",
    title: "Topology Under Fire",
    codename: "BLAST MAP",
    role: "Incident Commander",
    difficulty: "elite",
    description: "A problem opened. Before you touch anything, map the full blast radius using Smartscape. Know what's affected before you act.",
    briefing: "An active problem is spreading. Your first instinct might be to jump into the Problems app and start investigating. Don't. The most dangerous thing an Incident Commander can do is act without understanding scope. Open Smartscape, use the Problems Graph, and map the full blast radius before you touch anything. Use the Dynatrace Playground at https://playground.apps.dynatrace.com",
    timerSeconds: 480,
    status: "available",
    prerequisites: [],
    disciplines: [
    ...
    topics: ["problems", "infrastructure", "dt-intelligence"],
    category: "incident-response",
    apps: ["Smartscape"],
    checkpoints: [
      {
        ...
        instruction: "Open the Smartscape app. How many pre-built views are available on the Smartscape home screen?",
        hint: "The Smartscape home screen shows several pre-configured topology views. Count every view option listed — including Grail, Infrastructure, Problems Graph, Service Dependency Graph, and Kubernetes.",
        type: "multiple-choice",
        choices: ["3", "4", "5", "6 or more"],
      ...
      {
        id: "cp2",
        title: "Open the Problems Graph",
        instruction: "Click 'Open problem graph' in Smartscape. What does the Problems Graph show that the standard Problems app list does not?",
        hint: "The Problems Graph is a visual topology view. Think about what it adds beyond a simple list of problems.",
        type: "multiple-choice",
        choices: [
          "More detailed timestamps for each problem",
          "The causal relationships and blast radius between problems as a visual dependency graph",
          "A list of affected users for each problem",
          "The DQL queries used to detect each problem",
        ],
        correctChoice: "The causal relationships and blast radius between problems as a visual dependency graph",
        points: 200,
      },
      {
        id: "cp3",
        title: "Service Dependency Graph",
        instruction: "Go back to Smartscape and open the Service Dependency Graph. Approximately how many services are shown in the graph?",
        hint: "The Service Dependency Graph shows all monitored services and their connections. The total count is large — think in terms of hundreds.",
        type: "multiple-choice",
        choices: [
          "Less than 20",
          "20 to 50",
        ...
        correctChoice: "More than 100",
        points: 150,
      },
      {
        id: "cp4",
        title: "Blast Radius First",
        instruction: "An alert fires on a single service. As Incident Commander, what is the correct first action before any remediation?",
        hint: "The whole point of Smartscape is to answer this question before you act. What do you need to know first?",
        type: "multiple-choice",
        choices: [
          "Restart the affected service immediately",
          "Map the full blast radius — identify all upstream and downstream dependencies before touching anything",
          "Escalate to the on-call engineer",
          "Check the logs for the affected service",
        ],
        correctChoice: "Map the full blast radius — identify all upstream and downstream dependencies before touching anything",
        points: 200,
      },
    ],
  },
  {
    id: "mission-silent-anomaly",
    title: "The Silent Anomaly",
    codename: "DARK SIGNAL",
    role: "SRE",
    difficulty: "elite",
    description: "No alert fired. No problem opened. But something changed 24 hours ago. Use Assist to find what Davis hasn't caught yet.",
    briefing: "The most dangerous incidents are the ones nobody knows about yet. Davis AI opens problems when anomalies breach thresholds — but what about the signals that are degrading below the threshold? What about the metric drift that will become a problem in 6 hours? This mission teaches you to use Assist proactively — hunting for anomalies before they become incidents. Use the Dynatrace Playground at https://playground.apps.dynatrace.com",
    timerSeconds: 420,
    status: "available",
    prerequisites: [],
    disciplines: [
    ...
    topics: ["dt-intelligence", "problems", "metrics"],
    category: "incident-response",
    apps: ["Dynatrace Assist"],
    checkpoints: [
      {
        id: "cp1",
        title: "The Proactive Prompt",
        instruction: "Which Assist prompt would best surface anomalies that haven't triggered a Davis problem yet?",
        hint: "You want Assist to look beyond open problems — into metric trends, SLO burn rates, and signals that are degrading but haven't breached thresholds.",
        type: "multiple-choice",
        choices: [
          "Show me all open problems",
          "Are there any anomalies, metric degradations, or SLO burns in the last 24 hours that have not opened a Davis problem?",
          "What problems fired in the last 24 hours?",
          "Show me infrastructure health",
        ],
        correctChoice: "Are there any anomalies, metric degradations, or SLO burns in the last 24 hours that have not opened a Davis problem?",
        points: 200,
      },
      {
        id: "cp2",
        title: "Why Silent Anomalies Are Dangerous",
        instruction: "A metric has been degrading for 6 hours but hasn't crossed the Davis anomaly threshold. Why is this operationally dangerous?",
        hint: "Think about what happens when a slow degradation continues unchecked — and what state your team is in when it finally breaches.",
        type: "multiple-choice",
        choices: [
          "It isn't dangerous — if Davis didn't open a problem, there's nothing to worry about",
          "The degradation will self-correct before it becomes a problem",
          "By the time it breaches the threshold and Davis opens a problem, the degradation has been compounding for hours and the blast radius is larger than if caught early",
          "Davis always catches every anomaly before users notice",
        ],
        correctChoice: "By the time it breaches the threshold and Davis opens a problem, the degradation has been compounding for hours and the blast radius is larger than if caught early",
        points: 250,
      },
      {
        id: "cp3",
        title: "Forecasting as Prevention",
        instruction: "Which Assist prompt uses the Forecasting Agent to predict whether a current metric trend will become a problem?",
        hint: "The Forecasting Agent needs a specific metric, a prediction window, and historical data context to generate a forecast.",
        type: "multiple-choice",
        choices: [
          "Will frontend-high-cpu have a problem soon?",
          "Predict CPU usage for frontend-high-cpu over the next 24 hours based on the last 30 days of data — will it reach saturation?",
          "show me CPU forecast",
          "What is the CPU trend?",
        ],
        correctChoice: "Predict CPU usage for frontend-high-cpu over the next 24 hours based on the last 30 days of data — will it reach saturation?",
        points: 200,
      },
      {
        id: "cp4",
        title: "Proactive vs Reactive SRE",
        instruction: "What is the fundamental difference between a reactive SRE workflow and a proactive one in the context of Dynatrace Assist?",
        hint: "Think about when each type of SRE opens Assist and what they ask it.",
        type: "multiple-choice",
        choices: [
          "Reactive SREs use Assist more than proactive SREs",
          "Proactive SREs use Assist only after an incident is resolved",
          "Reactive SREs wait for Davis to open a problem before investigating. Proactive SREs use Assist to hunt for degrading signals before thresholds are breached",
          "There is no difference — both use the same Assist prompts",
        ],
        correctChoice: "Reactive SREs wait for Davis to open a problem before investigating. Proactive SREs use Assist to hunt for degrading signals before thresholds are breached",
        points: 150,
      },
    ],
  },
  {
    id: "mission-close-the-loop",
    title: "Close the Loop",
    codename: "AUTOMATE OR DIE",
    role: "Platform Engineer",
    difficulty: "elite",
    description: "The same alert has fired 47 times. Every time, an engineer investigates manually. Use Assist to design the workflow that ends the toil — with a human in the loop.",
    briefing: "Toil is the enemy of reliability. If you're manually responding to the same alert repeatedly, you're not doing SRE — you're doing ops theater. The fix is automation with judgment: a workflow that detects, notifies, waits for human approval, then remediates. Assist can design it. Your job is to ask the right questions and understand what gets built. Use the Dynatrace Playground at https://playground.apps.dynatrace.com",
    timerSeconds: 420,
    status: "available",
    prerequisites: [],
    disciplines: [
      { track: "platform-engineer", xp: 250 },
      { track: "sre", xp: 150 },
    ],
    topics: ["automation", "dt-intelligence", "problems"],
    category: "configuration",
    apps: ["Dynatrace Assist", "Workflows"],
    checkpoints: [
      {
        id: "cp1",
        title: "The Workflow Design Prompt",
        instruction: "Which prompt asks Assist to design a complete auto-remediation workflow with a human approval gate?",
        hint: "A good workflow design prompt specifies: the trigger condition, the notification, the approval gate, and the remediation action — all in one prompt.",
        type: "multiple-choice",
        choices: [
          "create a workflow for CPU alerts",
          "Generate a Dynatrace workflow that fires when CPU usage exceeds 90% on any host for more than 5 minutes, sends a Slack notification with context, waits for human approval, then restarts the affected process group",
          "how do I automate CPU remediation",
          "show me workflow templates",
        ],
        correctChoice: "Generate a Dynatrace workflow that fires when CPU usage exceeds 90% on any host for more than 5 minutes, sends a Slack notification with context, waits for human approval, then restarts the affected process group",
        points: 200,
      },
      {
        id: "cp2",
        title: "Approval Gate Sequence",
        instruction: "In a human-in-the-loop workflow, what is the correct sequence of steps?",
        hint: "The human needs enough information to make an informed approval decision. That information must arrive before the gate, not after.",
        type: "multiple-choice",
        choices: [
          "Trigger → Remediation → Notification → Approval gate",
          "Trigger → Approval gate → Notification → Remediation",
          "Trigger → Notification with context → Approval gate → Remediation",
          "Approval gate → Trigger → Notification → Remediation",
        ],
        correctChoice: "Trigger → Notification with context → Approval gate → Remediation",
        points: 200,
      },
      {
        id: "cp3",
        title: "Workflow Trigger Type",
        instruction: "For a workflow that fires when CPU exceeds 90% for more than 5 minutes, which Dynatrace Workflows trigger type should you use?",
        hint: "You are triggering on a sustained metric threshold — not a problem event, not a schedule, not an external webhook.",
        type: "multiple-choice",
        choices: [
          "Schedule trigger",
        ...
        correctChoice: "Metric event trigger",
        points: 150,
      },
      {
        id: "cp4",
        title: "When NOT to Automate",
        instruction: "A cascade failure is affecting 8 services simultaneously. An automated workflow detects CPU saturation on one of them and is about to restart the process. What should stop it?",
        hint: "Think about what restarting a process does during a cascade — and why the approval gate exists.",
        type: "multiple-choice",
        choices: [
          "Nothing — the workflow should proceed automatically to resolve faster",
          "The approval gate — a human should review whether restarting during a cascade will help or make things worse before any action runs",
          "The workflow should be disabled during incidents",
          "Davis AI will automatically pause all workflows during a cascade",
        ],
        correctChoice: "The approval gate — a human should review whether restarting during a cascade will help or make things worse before any action runs",
        points: 250,
      },
    ],
  },
